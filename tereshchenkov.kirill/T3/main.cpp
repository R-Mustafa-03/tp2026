...
Polygon parsePolygon(const std::string& line) {
  ...
  for (int i = 0; i < n; ++i) {
    char op, sem, cl;
    int x, y;
    if (iss >> op >> x >> sem >> y >> cl && op == '(' && sem == ';' && cl == ')') {
    ...
    try {
      if (cmd == "AREA") {
        ...
        if (sub == "ODD")
          ...
          res = std::accumulate(container.begin(), container.end(), 0.0,
                                AreaSummator([](const Polygon&){return true;})) /
                                container.size();
        } else {
        ...
        std::string sub; std::cin >> sub;
        if (sub == "ODD")
          std::cout << std::count_if(container.begin(), container.end(), isVertexCountOdd) << '\n';
        else if (sub == "EVEN")
          std::cout << std::count_if(container.begin(), container.end(), isVertexCountEven) << '\n';
        else {
          size_t n = std::stoul(sub);
          if (n < 3) throw std::invalid_argument("");
        ...
        SeqState res = std::accumulate(container.begin(), container.end(), SeqState(target),
                                       std::bind(&SeqState::operator+, _1, _2));
        std::cout << res.current_max << '\n';
      } else {
